...
<section class="grid place-items-center">
	<div class="grid grid-cols-12 gap-4 w-[90]">
		<div
			class="col-span-10"
			use:viewport
		...
		>
			{#key currentGame}
				<div class="card image-full">
					<figure in:fly={{ x: 100, duration: 1000 }} class="">
		...
		<div class="col-span-2">
			<div class="flex flex-col h-full justify-between">
				{#each CAROUSEL_GAMES as game, i}
					<button
						class="btn btn-ghost flex-col py-8 pr-8 gap-4 content-start align-middle relative"
						on:click={() => {
					...
					>
						<img class="w-10 rounded-md z-10" src={game.img.sm} alt={game.name} />
